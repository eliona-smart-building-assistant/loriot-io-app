...
DROP SCHEMA IF EXISTS loriot_io CASCADE;

DELETE FROM heap
WHERE asset_id IN (
	SELECT asset_id
	FROM asset
	WHERE asset_type LIKE E'loriot\\_%'
);
...
WHERE widget_id IN (
	...
	WHERE public.dashboard.name LIKE 'Loriot%'
);
...
WHERE dashboard_id IN (
	...
	WHERE name LIKE 'Loriot%'
);

DELETE FROM public.dashboard
WHERE name LIKE 'Loriot%';

-- DELETE FROM eliona_app WHERE app_name = 'loriot-io';
-- DELETE FROM eliona_store WHERE app_name = 'loriot-io';
